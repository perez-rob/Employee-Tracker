...
class Queries {
    ...
    // THE DEPARTMENT NAME ATTACHED TO IT WHICH NEEDS TO BE REMOVED TO USE THE STRING CORRECTLY
    {
      ...
      (err, res) => {
        ...
        let titleTbl = cTable.getTable([{
          ...
          Role: manIndex[manager.split(' - ')[0]][1],
  ...
  getRoleChoices() {
    ...
    this.connection.query(
      ...
      (err, res) => {
        ...
        choiceArr = res.reduce((arr, item) => {
          let newItem = chalk`{cyan ${item.title} - ${item.Department}}`;
          arr.push(newItem);
          return arr;
        }, []);
        console.log(choiceArr)
  ...
  displayEmployeeByRole(role, roleIndex) {
    ...
    LEFT JOIN employee m ON e.manager_id = m.id
    WHERE ?;`,
    // THE .split(' - ') METHODS IN THE FOLLOWING LINES ARE NEEDED BECAUSE THE role PARAMETER HAS
    // THE DEPARTMENT NAME ATTACHED TO IT WHICH NEEDS TO BE REMOVED TO USE THE STRING CORRECTLY
    {
      "role.id": roleIndex[role.split(' - ')[0]][0]
    },
      (err, res) => {
        if (err) console.log(err);
        let titleTbl = cTable.getTable([{
          Role: role.split(' - ')[0],
          Salary: roleIndex[role.split(' - ')[0]][1],
          Department: roleIndex[role.split(' - ')[0]][2]
        }]);
        let table = cTable.getTable(res);
        console.log(' ');
        console.log(titleTbl);
