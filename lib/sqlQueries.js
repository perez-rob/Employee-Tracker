...
class Queries {
  ...
  // ==================================================================
  // COMBINE REDUCE FUNCTIONS
  getRoleChoices(forUpdate, otherInfo) {
    let choiceArr;
    let roleIndex;
    this.connection.query(
      `SELECT 
      ...
      (err, res) => {
        ...
        // console.log(choiceArr)
        forUpdate ? this.parentObj.updateEmployeeRole(choiceArr, roleIndex, otherInfo) : this.parentObj.viewEmployeeRole(choiceArr, roleIndex);
      }
    )    
  }
  ...
  getEmployeeChoices(switchFn) {
    ...
    ORDER BY
      ...
      (err, res) => {
        ...
        switch(switchFn) {
          case 'REMOVE':
            this.parentObj.removeEmployee(choiceArr, employeeIndex);
            break;
          case 'ROLE':
            this.parentObj.updateEmployeeFirst(choiceArr, employeeIndex, switchFn);
            break;
          case 'MANAGER':
            this.parentObj.removeEmployee(choiceArr, employeeIndex, switchFn);
            break;
  ...
  updateEmployeeRoleDB(role, roleIndex, otherInfo) {
    // console.log('1', otherInfo)
    // console.log('2', roleIndex)
    let employee = otherInfo[0];
    let employeeIndex = otherInfo[1];
    // console.log(role)
    this.connection.query(
      ...
      (err, res) => {
        if (err) console.log(err);
        console.log(chalk`\n\n{magentaBright   Successfully updated employee entry: \n\n}{magenta.bold      ${otherInfo[2][0]} ${otherInfo[2][1]} -- ${role} }\n\n{yellow =======================================================}\n\n`);
          this.parentObj.navQuestion();
