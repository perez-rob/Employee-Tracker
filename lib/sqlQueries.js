...
class Queries {
  constructor(parentObj) {
    ...
    this.connection = mysql.createConnection({
      ...
      port: 3306,
      user: "root",
      // Password is imported from .env, which is listed in .gitignore so it is not exposed publicly
      // You can enter your own SQL password to use on your machine
  ...
  displayEmployeeAll() {
    ...
    ORDER BY e.first_name;`,
      (err, res) => {
        if (err) console.log(err);
        // BELOW, THE SPLIT AND REJOIN ARE USED TO INDENT THE TABLE SO IT IS NOT RIGHT AGAINST THE LEFT SIDE OF THE CONSOLE
      ...
      (err, res) => {
        ...
        managerIndex = res.reduce((obj, item) => {
          if (!checklist.includes(item.Department)) {
            ...
            choiceArr.push(
              new inquirer.Separator(chalk`{magenta  ${item.Department}}`)
            );
            choiceArr.push(new inquirer.Separator());
      ...
      (err, res) => {
        ...
        managerIndex = res.reduce((obj, item) => {
          if (!checklist.includes(item.title)) {
            ...
            managerChoices.push(
              new inquirer.Separator(chalk`{magenta  ${item.title}}`)
            );
            managerChoices.push(new inquirer.Separator());
        ...
        this.parentObj.addEmployeeManager(
          managerChoices,
          managerIndex,
          roleIndex,
          otherAnswers
  ...
  addEmployeeToDB(managerIndex, roleIndex, managerAnswer, otherAnswers) {
    ...
    manager = manager.includes("No Manager")
      ? "No Manager"
  ...
  handleUpdateRole(target, employeeIndex) {
    ...
    this.connection.query(
      ...
      (err, res) => {
        ...
        this.parentObj.updateEmployeeRole(
          choiceArr,
          roleIndex,
          target,
          employeeIndex
      ...
      (err, res) => {
        ...
        choiceArr = res.reduce((arr, item) => {
          ...
          if (!checklist.includes(subSeparator)) {
            arr.push(new inquirer.Separator(" "));
            arr.push(
              new inquirer.Separator(chalk`{magenta   ${subSeparator}}`)
            );
            arr.push(new inquirer.Separator(" "));
        ...
        // console.log(choiceArr)
        this.parentObj.updateEmployeeManager(
          choiceArr,
          managerIndex,
          target,
          employeeIndex
      ...
      (err, res) => {
        ...
        removeIndex = res.reduce((obj, item) => {
          if (!checklist.includes(item.department)) {
            choiceArr.push(new inquirer.Separator(" "));
            choiceArr.push(
              new inquirer.Separator(chalk`{white ${item.department}}`)
            );
            choiceArr.push(new inquirer.Separator());
            checklist.push(item.department);
          }
  ...
  removeEmployeeFromDB(answer, removeIndex) {
    let employee = answer.includes("CANCEL")
      ? "CANCEL"
      : answer.substr(9, answer.length - 14);
    if (employee == "CANCEL") {
      console.log(" ");
      this.parentObj.navQuestion();
    } else {
  ...
  getAllRoles() {
    this.connection.query(
      `SELECT role.id AS ID, title AS Role, salary AS Salary, NAME AS department
      FROM role
  ...
  getRolesByDepartment() {
    this.connection.query(
      ...
      (err, res) => {
        ...
        this.parentObj.rolesByDepartment(choiceArr).then((answer) => {
  ...
  addNewRole() {
    this.connection.query(
      ...
      (err, res) => {
        ...
        res.map((choice) => (depIndex[choice.department] = choice.id));
        let choiceArr = res.map((choice) => chalk`{cyan ${choice.department}}`);
        this.parentObj.addRole(choiceArr).then((answers) => {
  ...
  removeRole() {
    this.connection.query(
      ...
      (err, res) => {
        ...
        let roleIndex = {};
        res.map(
          (choice) => (roleIndex[choice.title] = [choice.id, choice.department])
        );
        let checklist = [];
        let choiceArr = [];
        res.map((item) => {
        ...
        this.parentObj.removeRole(choiceArr).then((answers) => {
          if (answers.rmRole == chalk`{redBright CANCEL}`) {
            ...
            this.connection.query(
              ...
              (err, res) => {
                if (err) console.log(err);
                console.log(
                  chalk`\n\n{redBright   Successfully removed ROLE from database: \n\n}{red.bold      ${role} from ${roleIndex[role][1]} }\n\n{yellow =======================================================}\n\n`
                );
                this.parentObj.navQuestion();
              }
            );
  ...
  getExpenses() {
    this.connection.query(
      ...
      (err, res) => {
        ...
        res.map((choice) => (depIndex[choice.department] = choice.id));
        let choiceArr = res.map((choice) => chalk`{cyan ${choice.department}}`);
        this.parentObj.personnelExpenses(choiceArr).then((answer) => {
          let dep = answer.byDepartment;
          dep = dep.substr(5, dep.length - 10);
          this.connection.query(
            `SELECT
            ...
            UNION 
            SELECT
              REPLACE(d.name,'Accounting','Company Total'),
              SUM(salary) AS Total_Expenses,
              COUNT(e.id) AS  All_Employess
            FROM employee e
            LEFT JOIN role r ON r.id = e.role_id
            LEFT JOIN department d ON d.id = r.department_id;
              `,
            ...
            (err, res) => {
              if (err) console.log(err);
              let titleTbl = cTable.getTable(res).split(`\n`);
              let titleStyle = titleTbl.map(
                (line) => chalk`{white.bold ${line}}`
              );
              console.log(" ");
              console.log(`   ${titleStyle.join(`\n   `)}`);
              this.connection.query(
                `SELECT
                  r.title AS Role,
                  SUM(salary) AS Expenses,
  ...
  addNewDepartment() {
    this.parentObj.addDepartment().then((answer) => {
      // let dep = (answer.departmentName.includes('CANCEL') ? 'CANCEL' : answer.substr(9, answer.length - 14));
      if (answer.departmentName.toUpperCase() == "CANCEL") {
        console.log(" ");
        this.parentObj.navQuestion();
      } else {
        this.connection.query(
        ...
        `,
          ...
          (err, res) => {
            if (err) console.log(err);
            console.log(
              chalk`\n\n{greenBright   Successfully added NEW DEPARTMENT to database: \n\n}{green.bold      ${answer.departmentName} }\n\n{yellow =======================================================}\n\n`
            );
            this.parentObj.navQuestion();
          }
        );
  ...
  removeDepartment() {
    this.connection.query(
      `SELECT id, department.name AS department
      FROM department;
      `,
      (err, res) => {
        if (err) console.log(err);
        ...
        let choiceArr = [];
        res.map((item) => {
          choiceArr.push(chalk`{cyan ${item.department}}`);
        });
        this.parentObj.removeDepartment(choiceArr).then((answers) => {
          if (answers.rmDep == chalk`{redBright CANCEL}`) {
            console.log(" ");
            this.parentObj.navQuestion();
            return;
          } else {
            let dep = answers.rmDep;
            dep = dep.substr(5, dep.length - 10);
            this.connection.query(
              ...
              (err, res) => {
                if (err) console.log(err);
                console.log(
                  chalk`\n\n{redBright   Successfully removed DEPARTMENT from database: \n\n}{red.bold      ${dep}}\n\n{yellow =======================================================}\n\n`
                );
                this.parentObj.navQuestion();
              }
            );
