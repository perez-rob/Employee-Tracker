...
class Queries {
    ...
    this.connection.query(
      ...
      (err, res) => {
        ...
        choiceArr = res.reduce((arr, item) => {
          ...
          let newItem = chalk`{magenta   ${item.title}}`;
          arr.push(newItem);
          return arr;
        }, []);
        // console.log(choiceArr)
  ...
  updateRoleDB(role, employee, roleIndex, employeeIndex) {
    this.connection.query(
      ...
      (err, res) => {
        if (err) throw err;
        console.log(chalk`\n\n{cyan   Successfully updated {magenta Role} in database: \n\n}{white.bold      ${employee.split(', ')[1]} ${employee.split(', ')[0]} }{cyan -- }{magenta ${role} }\n\n{yellow =======================================================}\n\n`);
        this.parentObj.navQuestion();
      }
    );
  }
  ...
  updateManagerDB(manager, employee, managerIndex, employeeIndex) {
    let firstName = manager.split(', ')[1];
    let lastName = manager.split(', ')[0];
    this.connection.query(
      'UPDATE employee SET ? WHERE ?',
      [
        {
          manager_id: managerIndex[manager][0],
        },
        {
          id: employeeIndex[employee][0],
        },
      ...
      (err, res) => {
        if (err) throw err;
        console.log(chalk`\n\n{cyan   Successfully updated {magenta Manager} in database: \n\n}{white.bold      ${employee.split(', ')[1]} ${employee.split(', ')[0]} }{cyan is working under }{magenta ${firstName} ${lastName} }\n\n{yellow =======================================================}\n\n`);
        this.parentObj.navQuestion();
      }
    );
  }
