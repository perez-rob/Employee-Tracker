  ...
  getExpenses() {
    this.connection.query(
      ...
      (err, res) => {
        ...
        res.map((choice) => depIndex[choice.department] = choice.id);
        let choiceArr = res.map((choice) => chalk`{cyan ${choice.department}}`);
        this.parentObj.personnelExpenses(choiceArr)
          .then((answer) => {
            let dep = answer.byDepartment;
            dep = dep.substr(5, dep.length - 10);
            this.connection.query(
            ...
            LEFT JOIN department d ON d.id = r.department_id;
              ...
              (err, res) => {
                ...
                let titleStyle = titleTbl.map((line) => chalk`{white.bold ${line}}`);
                console.log(' ');
  ...
  removeDepartment() {
    this.connection.query(
      ...
      (err, res) => {
        ...
        res.map((choice) => depIndex[choice.department] = choice.id);
        let choiceArr = []; 
        res.map((item) => {choiceArr.push( chalk`{cyan ${item.department}}`)
        })
        this.parentObj.removeDepartment(choiceArr)
          .then((answers) => {
