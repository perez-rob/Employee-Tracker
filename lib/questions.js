      ...
      .prompt([
        {
          ...
          choices: [ new inquirer.Separator(' '), new inquirer.Separator(chalk`{white.bold Employees}`), new inquirer.Separator(), chalk`{cyan.bold view ALL}`, chalk`{cyan.bold view by DEPARTMENT}`, chalk`{cyan.bold view by MANAGER}`, chalk`{cyan.bold view by ROLE}`, chalk `{green.bold NEW employee}`, chalk `{magenta.bold update ROLE}`, chalk `{magenta.bold update MANAGER}`, chalk `{red.bold DELETE employee}`, new inquirer.Separator(" "), new inquirer.Separator(chalk`{white.bold Roles}`), new inquirer.Separator(), chalk `{cyan.bold View All}`, chalk `{cyan.bold View by DEPARTMENT}`, chalk `{green.bold ADD Role}`, chalk `{red.bold REMOVE Role}`, new inquirer.Separator(" "), new inquirer.Separator(chalk`{white.bold Departments}`), new inquirer.Separator(), chalk `{cyan.bold View ALL}`, chalk `{cyan.bold View Personnel Expenses}`, chalk `{green.bold Add Department}`, chalk `{red.bold Remove Department}`, new inquirer.Separator(' '), new inquirer.Separator(), chalk `{red.bold      QUIT}`, new inquirer.Separator()]
        }
      ])
      .then((answers) => {
        switch(answers.nav){
      ...
      .prompt([
        {
          ...
          choices: [new inquirer.Separator(' '), chalk`{redBright CANCEL}`, new inquirer.Separator(), ...choiceArr, new inquirer.Separator()]
        },
        {
          name: 'roleTitle',
          type: 'input',
        ...
        },
        {
          name: 'roleSalary',
          type: 'input',
          message(answers) { return chalk`{yellow What is the salary for ${answers.roleTitle}?}\n   `},
          when(answers) {
            if (answers.departmentID == chalk`{redBright CANCEL}`){
              return false;
            }
